...
class CalculateScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.toll_value = TextInput(multiline=False, input_type='number', disabled=True)
    ...
    def calculate_fare(self, instance):
        ...
        if self.value_per_km.text:
            valor_por_km = float(self.value_per_km.text)
        else:
        ...
        if resultado:
            ...
            self.result.text = f"Distância: {distancia}, Duração: {duracao}, Valor Estimado: {tarifa}"
        else:
